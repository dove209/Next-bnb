...
const RoomMain: React.FC = () => {
    ...
    const getRoomListInfo = `${rooms.length}개의 숙소 ${checkInDate ? `${checkInDate ? format(new Date(checkInDate), 'MM월 dd일') : ''}` : ''} ${checkOutDate ? `${checkOutDate ? format(new Date(checkOutDate), '- MM월 dd일') : ''}` : ''}`;
